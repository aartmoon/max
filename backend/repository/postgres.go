...
func (p Postgres) Migrate(ctx context.Context) error { _, err := p.Pool.Exec(ctx, schema); return err }
...
func (p Postgres) List(ctx context.Context, user string) ([]domain.Request, error) {
	rows, err := p.Pool.Query(ctx, selectRequest+`WHERE r.user_id=$1 ORDER BY r.created_at DESC,r.id DESC`, user)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (p Postgres) Get(ctx context.Context, id, user string) (domain.Request, error) {
	return scan(p.Pool.QueryRow(ctx, selectRequest+`WHERE r.id=$1 AND r.user_id=$2`, id, user))
}
func (p Postgres) History(ctx context.Context, id, user string) ([]domain.RequestStatusHistory, error) {
	...
	rows, err := p.Pool.Query(ctx, `SELECT id,request_id::text,status,created_at FROM request_status_history WHERE request_id=$1 ORDER BY id`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&h.ID, &h.RequestID, &h.Status, &h.CreatedAt); err != nil {
			return nil, err
		}
...
func (p Postgres) Transition(ctx context.Context, id, user string, next func(string) (string, error)) (domain.Request, error) {
	...
	defer tx.Rollback(ctx)
	r, err := scan(tx.QueryRow(ctx, selectRequest+`WHERE r.id=$1 AND r.user_id=$2 FOR UPDATE OF r`, id, user))
	if err != nil {
		return r, err
	}
	...
	if _, err = tx.Exec(ctx, `INSERT INTO request_status_history(request_id,status) VALUES($1,$2)`, id, r.Status); err != nil {
		return r, err
	}
	return r, tx.Commit(ctx)
}
func (p Postgres) Photo(ctx context.Context, id, user string) ([]byte, string, error) {
	var b []byte
	var mime string
	err := p.Pool.QueryRow(ctx, `SELECT photo,photo_type FROM requests WHERE id=$1 AND user_id=$2`, id, user).Scan(&b, &mime)
